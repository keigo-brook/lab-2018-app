...
STATUS_TOPIC="road-load-status"
IMG_TOPIC='capImgLab'
RESULT_TOPIC = 'road-result'
DEFAULT_RESULT_TOPIC = RESULT_TOPIC + '-default'
TASK_SERVER_NAME = 'road-damage-listener'
TASK_SERVER_PORT = 8199
...
def task(message):
    ...
    # send to detector
    file_type = '.' + dictrx['type']
    sock.send(file_type.encode())
    if not wait_ok(sock):
        print('file type send failed')
        return
    ...
    if not wait_ok(sock):
        print('file length send failed')
        return

    # send actual image data
    sock.send(data)
    time_log['send_to_road_detect_finish'] = time.time()
    ...
    # receive YOLO result
    result = "{{\"file_name\": {}, \"result\": ".format(image_file_name)
    rec_data_len = 0
    while True:
        data = sock.recv(4096)
        data = data.decode()
    ...
    time_log['receive_from_road_detect_done'] = time.time()
...
if __name__ == '__main__':
    ...
    produce(STATUS_TOPIC, "start rotad detect load balancer at {}".format(time.time()))

    if len(sys.argv) < 2:
        print("./road_load_balancer.py num_of_workers")
        exit()
